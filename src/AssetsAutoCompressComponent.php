...
namespace weikaiii\yii2\assetsAuto;
...
class AssetsAutoCompressComponent extends Component implements BootstrapInterface
{
     ...
     * @var string
     */
    protected $_jsFileTimeMd5Path = 'jsFileTimeMd5';
    /**
     * @var string
     */
    protected $_cssFileTimeMd5Path = 'cssFileTimeMd5';
    /**
     ...
     * @return string
     * @author: weikai
     * @date: 2019/5/15 13:18
     */
    public function getJsFileTimeMd5Path()
    {
        return \Yii::getAlias($this->_webroot).'/'.$this->_jsFileTimeMd5Path;
    }
    /**
     * @return string
     * @author: weikai
     * @date: 2019/5/15 13:18
     */
    public function setJsFileTimeMd5Path($path)
    {
        $this->_jsFileTimeMd5Path = $path;
        return $this;
    }
    /**
     * @return string
     ...
     * @date: 2019/5/15 13:18
     */
    public function getCssFileTimeMd5Path()
    {
        return \Yii::getAlias($this->_webroot).'/'.$this->_cssFileTimeMd5Path;
    }
...
JS
    ...
    protected function _processingJsFiles($files = [])
    {
        ...
        foreach (array_keys($files) as $k=>$v){
            if (file_exists($this->webroot.$v)){
                $oldJsPath = $this->webroot.$v;
                $oldJsFileTime[] = filemtime($oldJsPath);
            }
        }
        ...
        if (!empty($oldJsFileTime)){
            if ($this->checkJsFileChange($oldJsFileTime)){
     ...
     */
    public function checkJsFileChange($oldJsFileTime)
    {
            $fileTimeMd5 = md5(implode($oldJsFileTime));
            if (!file_exists($this->getJsFileTimeMd5Path())){
                $file = fopen($this->getJsFileTimeMd5Path(), "w");
                fwrite($file, $fileTimeMd5);
                fclose($file);
            }else{
                $file = fopen($this->getJsFileTimeMd5Path(),'r');
                $fMd5 = fread($file,filesize($this->getJsFileTimeMd5Path()));
                if ($fMd5 !== $fileTimeMd5){
                    $file = fopen($this->getJsFileTimeMd5Path(), "w");
                    fwrite($file, $fileTimeMd5);
                    fclose($file);
     ...
     */
    public function checkCssFileChange($oldJsFileTime)
    {
        $fileTimeMd5 = md5(implode($oldJsFileTime));
        if (!file_exists($this->getCssFileTimeMd5Path())){
            $file = fopen($this->getCssFileTimeMd5Path(), "w");
            fwrite($file, $fileTimeMd5);
            fclose($file);
        }else{
            $file = fopen($this->getCssFileTimeMd5Path(),'r');
            $fMd5 = fread($file,filesize($this->getCssFileTimeMd5Path()));
            if ($fMd5 !== $fileTimeMd5){
                $file = fopen($this->getCssFileTimeMd5Path(), "w");
                fwrite($file, $fileTimeMd5);
                fclose($file);
    ...
    protected function _processingCssFiles($files = [])
    {
        ...
        if (!empty($oldJsFileTime)){
            if ($this->checkCssFileChange($oldJsFileTime)){
                if (file_exists($rootUrl)) {
                    $resultFiles = [];
